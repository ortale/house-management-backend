...
export class HouseService {
    ...
    async addHouse(house: Omit<House, 'id'>): Promise<void> {
        ...
        const result = await db.query('INSERT INTO houses (name, email, rentAmount, astExpDate) VALUES (?, ?, ?)', [house.name, house.email, house.rentAmount, house.astExpDate]);
    }

    async updateHouse(house: House): Promise<void> {
        await db.query('UPDATE houses SET name = ?, email = ?, rentAmount = ?, astExpDate = ? WHERE id = ?', [house.name, house.email, house.rentAmount, house.astExpDate, house.id]);
    }
