	...
	payments.forEach(async payment => {
		try {
			await transporter.sendMail({
				...
				to: process.env.IONOS_EMAIL, // List of recipients,
				cc: payment.email,
				subject: `${payment.houseName} - Rent Reminder Alert`, // Subject line
...
async function updateDueInvoicesSentEmail(id: any[]) {
	const [rows] = await db.query<RowDataPacket[]>(`
		UPDATE houses SET emailSentDate = CURDATE() WHERE id IN (${id})
	`);
	...
	const [rows] = await db.query<RowDataPacket[]>(`
		UPDATE houses SET astExpEmailSentDate = CURDATE() WHERE id IN (${id})
	`);
